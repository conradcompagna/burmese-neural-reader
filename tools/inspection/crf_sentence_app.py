...
def _bc_sent2features(
    tokens: List[str], extras: Optional[List[Dict[str, Any]]]
) -> List[Dict[str, str]]:
    return [_bc_extract_gap_features(tokens, i, extras) for i in range(len(tokens))]


def tokenize_boundary_text(
    text: str, short_line_ratio: float = 0.6
...
def spacy_ud_sentence_segment(
    text: str, *, spacy_model_path: str, newserver_url: Optional[str]
) -> Dict[str, Any]:
    """
...
def load_model(model_path: str) -> LoadedModel:
    ...
    if p.suffix.lower() == ".crfsuite":
        ...
        _MODEL_CACHE = LoadedModel(
            path=model_path, kind="crfsuite", model=tagger, win=win, meta=meta or {}
        )
        return _MODEL_CACHE
...
def crf_segment(model: LoadedModel, tokens: List[str]) -> Dict[str, Any]:
    ...
    """
    if not tokens:
        return {"tokens": [], "labels": [], "boundaries": [], "sentences": [], "marked_text": ""}
...
def ocr_structure_boundary_segment_crfsuite(model: LoadedModel, text: str) -> Dict[str, Any]:
    ...
    if p_threshold > 0.0:
        for i in range(len(predictions)):
            p_any = float(model.model.marginal("SENT_END", i)) + float(
                model.model.marginal("PARA_END", i)
            )
...
def api_segment():
    ...
    try:
        ...
        return jsonify({"error": repr(e), "traceback": traceback.format_exc()}), 500
