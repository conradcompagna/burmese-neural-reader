...
def _get_segmenter():
    global _SEGGER
    if _SEGGER is None:
        # app.py exposes the singleton via get_segmenter_instance().
...
def _init_newserver_pipeline() -> None:
    """
    Match app.py startup as closely as possible (without starting Flask).
    ...
    if _INITIALIZED:
        return
    # This is the exact startup path used by `python app.py` (minus app.run).
    ns.load_dictionary()
    try:
...
def _segment_for_app(tok: str) -> List[str]:
    ...
    - If Burmese: use the embedded DP segmenter from app.py.
    """
    tok = tok or ""
