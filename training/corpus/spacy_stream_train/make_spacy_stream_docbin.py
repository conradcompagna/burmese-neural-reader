...
def main() -> int:
    ...
    ap.add_argument(
        "--in_conllu", action="append", required=True, help="Repeatable. Input CoNLL-U path(s)."
    )
    ...
    train_docs = list(
        chunk_sentences_to_docs(train_sents, max_tokens_per_doc=int(args.max_tokens_per_doc))
    )
    dev_docs = list(
        chunk_sentences_to_docs(dev_sents, max_tokens_per_doc=int(args.max_tokens_per_doc))
    )
