...
def write_bigram_file(counts: Counter[Tuple[str, str]], out_path: Path) -> None:
    # ('w1', 'w2')\t<count>
    with out_path.open("w", encoding="utf-8") as f:
        for (w1, w2), c in sorted(counts.items(), key=lambda x: (-x[1], x[0][0], x[0][1])):
...
def write_phrase_bigram_file(counts: Counter[Tuple[str, str]], out_path: Path) -> None:
    ...
    with out_path.open("w", encoding="utf-8") as f:
        for (p1, p2), c in sorted(counts.items(), key=lambda x: (-x[1], x[0][0], x[0][1])):
...
def main() -> None:
    ...
    for p in in_paths:
        ...
        process_file_first_pass(
            p, unigram_counts, bigram_counts, phrase_unigram_counts, unknown_counts
        )
