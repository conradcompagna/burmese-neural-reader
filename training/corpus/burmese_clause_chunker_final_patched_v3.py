...
def classify_markers(marker_tokens, deprel=None):
    ...
    has_nominalizer = any(
        t["form"] in NOMINALIZERS and t["pos"] in ("PART", "NOUN") for t in non_aux
    )
...
def create_chunks(sentence, boundaries, include_moderate=True, include_relativizers=False):
    ...
    for b in cuts:
        ...
        if start <= end:
            ...
            if text.strip():
                chunks.append({"text": text, "type": b["type"], "trigger": b["trigger"]})
            prev_end = end
    ...
    if prev_end < len(sentence):
        ...
        if tokens:
            ...
            if text.strip():
                chunks.append({"text": text, "type": "remainder", "trigger": None})
...
if __name__ == "__main__":
    ...
    print(
        f"  TOTAL:               {len(STRONG_CLAUSE_MARKERS) + len(MODERATE_CLAUSE_MARKERS) + len(SENTENCE_FINAL_MARKERS) + len(NOMINALIZERS) + len(RELATIVIZERS) + len(AUXILIARIES)} markers"
    )

    # Test on corpus
    sentences = parse_conllu(
        "/mnt/user-data/uploads/myUDTree_ver1_0_conllu.pred", max_sentences=3000
    )
    ...
    for idx, (sent_i, sent, bounds) in enumerate(examples[:10]):
        ...
        for b in bounds:
            if b["type"] != "nominalizer":
                print(
                    f"   • {b['type'].upper():12} verb='{b['verb']}' → [{', '.join(b['markers'])}] trigger=⟨{b['trigger']}⟩"
                )
