...
def load_sequences(
    ...
    *,
    ...
    for rec in iter_jsonl(path):
        s = preprocess_record_to_sequence(
            rec, corpus=corpus, chronicle_allowed_enders=chronicle_allowed_enders
        )
        if s is None:
            continue
...
def word_features(seq: Sequence, i: int, win: int) -> dict[str, str]:
    ...
    for k in range(1, win + 1):
        if i - k >= 0:
            feats[f"-{k}:suf2"] = (
                seq.tokens[i - k][-2:] if len(seq.tokens[i - k]) >= 2 else seq.tokens[i - k]
            )
            feats[f"-{k}:suf3"] = (
                seq.tokens[i - k][-3:] if len(seq.tokens[i - k]) >= 3 else seq.tokens[i - k]
            )
        else:
            feats[f"BOS{k}"] = "1"
...
def eval_on_whitespace_text(model_path: Path, text_path: Path, win: int) -> None:
    ...
    seq = Sequence(
        tokens=tokens, upos=["X"] * len(tokens), tag=["X"] * len(tokens), y=[0] * len(tokens)
    )
    ...
    print(
        f"TARGET({text_path.name}): tokens={len(tokens)} pred_E={pred_total} zhaa_hit={zhaa_hit}/{zhaa_total}"
    )
...
def eval_thi_hit_rate_on_text(model_path: Path, text_path: Path, win: int) -> None:
    ...
    print(
        f"THI_HIT({text_path.name}): tokens={len(tokens)} pred_E={pred_total} thi_hit={thi_hit}/{thi_total}"
    )
    ...
    *,
    ...
    while i < len(raw_tokens):
        ...
        if rt == "<SENT_END>":
            ...
            if last_kept_idx is not None and not (
                next_tok is not None and next_tok in ignore_next_tokens
            ):
                gold.add(last_kept_idx)
...
def main() -> int:
    ...
    ap.add_argument(
        "--out_model",
        type=Path,
        default=Path("corpus/burmese_sentence_finalparticle_v6_thi_merge.crfsuite"),
    )
    ...
    ap.add_argument(
        ...
        type=Path,
        default=None,
        help="Optional <SENT_END>-marked file to add to training.",
    )
    ap.add_argument(
        "--extra_marked_repeat",
        type=int,
        default=0,
        help="Repeat extra_marked_train sequence N times (0 disables).",
    )
    ...
    chron_train = load_sequences(
        args.chron_train, corpus="chronicle", chronicle_allowed_enders=chron_allowed
    )
    myud_train = load_sequences(
        args.myud_train, corpus="myudtree", chronicle_allowed_enders=chron_allowed
    )
    alt_train = load_sequences(args.alt_train, corpus="alt", chronicle_allowed_enders=chron_allowed)

    chron_dev = load_sequences(
        args.chron_dev, corpus="chronicle", chronicle_allowed_enders=chron_allowed
    )
    myud_dev = load_sequences(
        args.myud_dev, corpus="myudtree", chronicle_allowed_enders=chron_allowed
    )
    ...
    aug_merge = (
        aug_merge_all[: max(0, int(args.thi_merge_max))] if int(args.thi_merge_max) > 0 else []
    )

    extra_seq_n = 0
    extra_seq: list[Sequence] = []
    if args.extra_marked_repeat and args.extra_marked_train and args.extra_marked_train.exists():
        s = sequence_from_marked_text(
            args.extra_marked_train, ignore_next_tokens=set(args.ignore_marked_next)
        )
        if s is not None:
