...
def make_X(tokens, pos, win, gram, modern_end, modern_rank, chron_end, chron_rank):
    return [
        word_features(tokens, pos, i, win, gram, modern_end, modern_rank, chron_end, chron_rank)
...
def concat_records(
    records: List[Tuple[List[str], List[int], List[dict]]], cmin: int, cmax: int, seed: int
):
...
def main():
    ...
    for toks, y, pos in train_seq2:
        X_train.append(
            make_X(toks, pos, args.win, gram, modern_end, modern_rank, chron_end, chron_rank)
        )
