...
def load_grammar_sentence_finals(grammar_tsv: str) -> Dict[str, set]:
    ...
    with open(grammar_tsv, "r", encoding="utf-8", errors="replace") as f:
        ...
        field_b = "burmese" if reader.fieldnames and "burmese" in reader.fieldnames else (reader.fieldnames[0] if reader.fieldnames else "burmese")
        field_c = "category" if reader.fieldnames and "category" in reader.fieldnames else (reader.fieldnames[1] if reader.fieldnames and len(reader.fieldnames) > 1 else "category")
        for r in reader:
...
def load_endword_unigrams(endword_tsv: str, label: str = "SENT_END", top_k: int = 300) -> Tuple[set, Dict[str, int]]:
    ...
    with open(endword_tsv, "r", encoding="utf-8", errors="replace") as f:
        reader = csv.DictReader(f, delimiter="\t")
        fld_label = "label" if reader.fieldnames and "label" in reader.fieldnames else (reader.fieldnames[0] if reader.fieldnames else "label")
        fld_rank = "rank" if reader.fieldnames and "rank" in reader.fieldnames else (reader.fieldnames[1] if reader.fieldnames and len(reader.fieldnames) > 1 else "rank")
        fld_uni = "unigram" if reader.fieldnames and "unigram" in reader.fieldnames else (reader.fieldnames[2] if reader.fieldnames and len(reader.fieldnames) > 2 else "unigram")
        for r in reader:
...
def word_features(tokens, pos, i, win, gram, modern_end, modern_rank, pooled_end, pooled_rank, chron_end, chron_rank):
    tok = tokens[i]
    n = strip_punct(tok)
    p = pos[i] if pos else {}
...
def make_X(tokens, pos, win, gram, modern_end, modern_rank, pooled_end, pooled_rank, chron_end, chron_rank):
    return [word_features(tokens, pos, i, win, gram, modern_end, modern_rank, pooled_end, pooled_rank, chron_end, chron_rank) for i in range(len(tokens))]
...
def main():
    ...
    for name in list(corp_train.keys()):
        corp_train[name] = concat_records(corp_train[name], args.concat_min, args.concat_max, seed=args.seed)
    ...
    for toks, y, pos in train_all:
        X_train.append(make_X(toks, pos, args.win, gram, modern_end, modern_rank, pooled_end, pooled_rank, chron_end, chron_rank))
    ...
    for name, seqs in corp_dev.items():
        ...
        for toks, y, pos in seqs:
            Xd.append(make_X(toks, pos, args.win, gram, modern_end, modern_rank, pooled_end, pooled_rank, chron_end, chron_rank))
            yd.append(make_y(y))
    ...
    out_obj = {
        ...
        "meta": {
            ...
            "dev_scores": {k: {"P": v[0], "R": v[1], "F1": v[2], "pred": v[3], "gold": v[4]} for k, v in scores.items()},
