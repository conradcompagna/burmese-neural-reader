...
def boundary_features(
    ...
    feats: Dict[str, str] = {
        ...
        "cur_line_short": "1" if (stats.line_short[cur_line] if stats.line_short else False) else "0",
        "cur_line_headerish": "1" if headerish(cur_st) else "0",
        "next_line_ratio": line_ratio_bucket(next_st),
        "next_line_short": "1" if (stats.line_short[next_line] if stats.line_short else False) else "0",
...
def label_chronicle_silver(text: str) -> Optional[LabeledDoc]:
    ...
    for i in range(len(y)):
        ...
        headerish = (my_ratio < 0.4) and ((cur_st.digit_islands + cur_st.punct_islands + cur_st.ascii_islands) >= 2)

        if is_eol and (line_short or headerish or ("\n\n" in gap)):
            y[i] = LABEL_PARA
            continue

        if token_type(prev_tok) == "MY" and (token_mixed_junk(prev_tok) or token_type(next_tok) != "MY"):
            y[i] = LABEL_SENT
            continue
    ...
    *,
    ...
    for _p in range(pcount):
        ...
        if para_islands and (rng.random() < attach_para_end_prob):
            para_islands[-1] = para_islands[-1] + pick_sep_token(rng, junk_pool, myanmar_punct_keep=myanmar_punct_keep)
...
def eval_docs(model_path: Path, docs: List[LabeledDoc], *, name: str) -> Dict[str, Any]:
    ...
    for lab in [LABEL_SENT, LABEL_PARA]:
        r = out[lab]
        print(f"{lab:8s} P={r['P']:.4f} R={r['R']:.4f} F1={r['F1']:.4f} (tp={r['tp']} fp={r['fp']} fn={r['fn']})")
...
def heuristic_target_test(model_path: Path, text_path: Path) -> None:
    ...
    print(f"heur_SENT gold={len(gold_sent)} pred={len(pred_sent)} hit={sent_hit} recall={sent_rec:.3f}")
    print(f"heur_PARA gold={len(gold_para)} pred={len(pred_para)} hit={para_hit} recall={para_rec:.3f}")
...
def main() -> int:
    ...
    ap.add_argument("--out_prefix", type=str, default="ocr_boundary_struct", help="Model name prefix under corpus/.")
    ...
    ap.add_argument("--attach_sep_prob", type=float, default=0.50, help="Prob of gluing separator to previous token at SENT_END.")
    ap.add_argument("--attach_para_end_prob", type=float, default=0.35, help="Prob of gluing junk to paragraph-final token.")
    ap.add_argument("--header_prob", type=float, default=0.10)
    ap.add_argument("--chron_upweight", type=int, default=2, help="Replicate chronicle silver blocks in training.")
    ...
    for name, c1, c2 in variants:
        ...
        train_one(out_model, train_docs, c1=c1, c2=c2, max_iter=int(args.max_iter), seed=int(args.seed))
        ...
        meta = {
            ...
            "metrics": {"synth_dev": synth_metrics, "chron_silver_dev": chron_metrics, "score": score},
