...
def main() -> int:
    ...
    ap.add_argument("--model", required=True, help="Path to *.crfsuite (ocr_boundary_struct_* model)")
    ...
    ap.add_argument("--p_threshold", type=float, default=0.0, help="Keep boundary only if P(SENT)+P(PARA) >= t")
    ap.add_argument("--min_gap", type=int, default=0, help="Suppress boundaries closer than this many islands")
    args = ap.parse_args()
    ...
    pred = apply_post_filters(tagger, pred, p_threshold=float(args.p_threshold), min_gap=int(args.min_gap))
