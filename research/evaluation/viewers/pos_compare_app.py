...
- Segments text using your newserver.py pipeline (normalize_burmese_for_segmentation + segment_with_pipeline).
...
Run:
  python pos_compare_app_endwords.py --newserver "C:\path\to\newserver.py"
...
def load_newserver(newserver_py: Path):
    if not newserver_py.exists():
        raise FileNotFoundError(f"newserver.py not found: {newserver_py}")
...
def build_app(newserver_mod, default_model_path: str, default_spans_key: str) -> Flask:
    ...
    if not hasattr(newserver_mod, "normalize_burmese_for_segmentation"):
        raise AttributeError("newserver.py missing normalize_burmese_for_segmentation()")
    if not hasattr(newserver_mod, "segment_with_pipeline"):
        raise AttributeError("newserver.py missing segment_with_pipeline()")
...
def main():
    ...
    p.add_argument("--newserver", type=str, required=True, help=r'Path to newserver.py (e.g. C:\...\newserver.py)')
    p.add_argument("--model", type=str, default=DEFAULT_MODEL, help="Default spaCy endwords spancat model path")
    p.add_argument("--spans-key", type=str, default=DEFAULT_SPANS_KEY, help="Default doc.spans key for spancat output (often 'sc')")
