...
def main() -> int:
    ...
    ap.add_argument("--strip_punct", action="store_true", help="Drop punctuation tokens and use last kept token.")
    ap.add_argument("--drop_non_myanmar", action="store_true", help="Also drop non-Myanmar, non-digit tokens.")
    ap.add_argument(
        "--tokenize",
    ...
    print(f"sentences={n_sent} counted={sum(ctr.values())} skipped={n_skipped} unique_endwords={len(ctr)}")
    for w, c in rows:
        print(f"{c}\t{w}")
