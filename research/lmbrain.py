...
class AdvancedSegmenter:
    ...
    def _build_spell_vocab(self) -> None:
        ...
        - Build a global frequency-sorted list for spell search.
        """
        cfg = self.config
        ...
        self.spell_vocab = vocab

        # Global frequency-sorted list for spell search
        def _freq(word: str) -> int:
            return UNIGRAM_COUNTS.get(word, 1)

        spell_list = sorted(vocab, key=_freq, reverse=True)
        self.spell_list = spell_list

        # Dummy index for compatibility; all queries use global list
        self.spell_index = {"*": spell_list}
    ...
    def suggest_spellings(
        ...
        # Global candidate pool (dictionary-driven)
        spell_list = getattr(self, "spell_list", None)
        if not spell_list:
            return []

        cand_pool = spell_list
        ...
        if stage1_candidates is None:
            ...
            # Use BK-tree for efficient fuzzy matching
            bk = getattr(self, "_bk_tree", None)
            if bk is None:
                # Fallback: BK-tree not built, use old brute-force scan
                # Tight length difference filter
                max_len_diff = max_edit_distance + 1

                for cand in cand_pool:
                    len_cand = len(cand)
                    if abs(len_word - len_cand) > max_len_diff:
                        continue

                    d = levenshtein_distance(word, cand)

                    # Skip exact matches (not a correction) or those too far away
    ...
    def suggest_spellings_distance_first(
        ...
        if bk is not None:
            ...
            for cand in spell_list:
                ...
                # Skip exact matches (d=0) or those too far away
                if d == 0 or d > max_edit_distance:
                    continue
