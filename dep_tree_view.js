...
(function(global) {
  ...
  function DepTreeView() {
    ...
    this.sentenceSource = "live";  // DEPLOYMENT: Always use live mode
    ...
    this.bottomUpChunkMode = true;  // DEPLOYMENT: Always on
  ...
  DepTreeView.prototype._ensureStyles = function() {
    ...
    style.textContent = ""
      ...
      + ".dep-tree-tooltip{position:fixed;background:#111827;color:#f9fafb;border:1px solid rgba(255,255,255,0.08);border-radius:8px;padding:8px 12px;font-size:12px;pointer-events:none;z-index:1200;max-width:92vw;box-shadow:0 8px 20px rgba(0,0,0,0.35);height:auto;overflow:visible;}"
      + ".dep-tree-tooltip .tt-tok{font-family:\"Pyidaungsu\",\"Noto Sans Myanmar\",\"Myanmar Text\",system-ui,sans-serif;font-size:17px;line-height:1.5;font-weight:600;color:#fff;margin:0 0 4px 0;}"
      + ".dep-tree-tooltip .tt-info{color:rgba(255,255,255,0.65);margin-top:4px;line-height:1.4;}"
  ...
  DepTreeView.prototype.init = function(opts) {
    ...
    if (opts.bottomUpChunkThreshold !== undefined) {
      this.bottomUpChunkThresholdValue = opts.bottomUpChunkThreshold;
    }
    // Debug mode options
  ...
  DepTreeView.prototype._buildDom = function() {
    ...
    // DEPLOYMENT: Source is always "live" - hide selector
    var sourceWrap = document.createElement("div");
    sourceWrap.className = "dep-tree-source-toggle";
    sourceWrap.style.display = "none";  // Hidden - only live mode
    ...
    // DEPLOYMENT: Sentence navigation hidden - live mode only shows current context
    var sentenceNav = document.createElement("div");
    sentenceNav.className = "dep-tree-sentence-nav";
    sentenceNav.style.display = "none";  // Hidden for deployment
    ...
    // Row 2: Toggle controls - DISABLED FOR DEPLOYMENT
    // All toggle controls are hidden; only zoom/reset remain visible
    var headerRow2 = document.createElement("div");
    headerRow2.className = "dep-tree-header-controls";
    headerRow2.style.display = "none";  // DEPLOYMENT: Hide all toggle controls
    ...
    headerRow3.style.display = "none";  // DEPLOYMENT: Hide all toggle controls
  ...
  DepTreeView.prototype._showTooltip = function(segIdx, x, y) {
    ...
    // Token at top
    var html = "<div class=\"tt-tok\">" + escapeHtml(node.token || "") + "</div>";
    // POS and dep in bright colors
    ...
    html += "</div>";
    html += "<div style=\"font-size:11px;color:#9ca3af;\">head " + escapeHtml(node.headSeg) + " | depth " + depth
    ...
    // DISABLED FOR DEPLOYMENT: Dictionary entries in tree view tooltip
    // Only POS tags, dep positions, and depth info are shown
    // (Dictionary lookup is still available in the main reader popup)
  ...
  DepTreeView.prototype.setBottomUpChunkThreshold = function(threshold) {
    this.bottomUpChunkThresholdValue = threshold;
    this._recomputeBottomUpChunks();
    if (this.lastHoverSeg !== null && this.highlightEnabled) {
      this._applyHighlight(this.lastHoverSeg);
    }
