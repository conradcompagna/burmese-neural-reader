...
class ReadingSRS:
    ...
    # ---- grading ---------------------------------------------------------
    def _update_sm2(self, review: ReviewState, quality: int, now: Optional[datetime] = None) -> None:
        """SM-2 style update.
...
def build_ud_overlay_for_segments(
    ...
    try:
        ...
        if precomputed_ner_ents is not None:
            precomputed_ner_ents = _filter_ner_entities_excluding_last_islands(segments, island_spans, precomputed_ner_ents)
        ...
        if overlay_json.get("ok"):
                    ...
                    if text_for_ner:
                        ...
                        if ner_nlp is not None:
                            try:
                                ...
                                overlay_json["ents"] = _stanza_ents_to_segments(text_for_ner, segments, ents)
                                stanza_ran = True
                            except Exception as e:
            ...
            if overlay_json.get("ents") is not None:
                overlay_json["ents"] = _filter_ner_entities_excluding_last_islands(segments, island_spans, overlay_json["ents"])
        return overlay_json
    except Exception as e:
        return {
            "ok": False,
...
def _run_stanza_ner_early(
    ...
    try:
        ...
        if ent_segments:
            ent_segments = _filter_ner_entities_excluding_last_islands(segments, island_spans, ent_segments)
...
class StackedDictionary:
    ...
    def add_layer(self, name: str, priority: int = 100, source_name: str | None = None) -> DictionaryLayer:
...
class LanguageModel:
    ...
    def _compute_unigram_costs(self) -> None:
        ...
        self.unigram_counts.clear()
        print(f"[INFO] Unigram costs: min={min_cost:.2f}, max={max_cost:.2f}, default={self.unigram_default_cost:.2f}")
...
class BurmeseSegmenter:
    ...
    def load_lm(self, unigram_path: str | Path,
                bigram_path: Optional[str | Path] = None) -> None:
        """Load language model files."""
        self.lm.load_unigram(unigram_path)
        if bigram_path:
            self.lm.load_bigram(bigram_path)
    def load_dictionary_tsv(self, path: str | Path, layer_name: str,
    ...
    def segment_with_info(self, text: str) -> list[dict]:
        ...
        if n == 0:
            return [{"text": text, "start": 0, "end": len(text),
                    "in_dict": False, "in_lm": False, "cost": 0.0, "entry": None}]
        ...
        def _compute_breakdown(seg_text: str, prev_word: Optional[str]) -> dict:
            ...
            "bigram_known": bool(bigram_known),
                ...
                "components": {
                    ...
                    "unknown_word_base": cfg.UNKNOWN_WORD_BASE_COST if branch in ("lm_only", "oov") else 0.0,
                    "lm_unigram": float(lm_val),
                    "lm_unigram_weighted": cfg.UNIGRAM_WEIGHT * float(lm_val),
                    "unigram_weight": cfg.UNIGRAM_WEIGHT,
                    "dict_discount": cfg.DICT_NO_LM_DISCOUNT if branch == "dict" and not lm_known else None,
                    "oov_penalty": (syllables - 1) * cfg.OOV_SYLLABLE_PENALTY if branch in ("lm_only", "oov") else 0.0,
                "bigram_score": bigram_score_next if bigram_cost_next is not None else 0.0,
                "bigram_reward": bigram_reward,
                "bigram_reward_weighted": cfg.BIGRAM_WEIGHT * bigram_reward if bigram_reward else 0.0,
            },
        }
        for i in range(n - 1, -1, -1):
            best_cost = float("inf")
...
def segmenter_add_user_word(headword: str, romanization: str = "",
                            pos: str = "", definition: str = "") -> bool:
...
def load_mmd_dict(path: Path, layer: DictionaryLayer) -> int:
    ...
    POS_TAGS = {'n', 'v', 'adj', 'adv', 'pron', 'part', 'particle', 'conj', 
                'prep', 'postp', 'ppm', 'interj', 'aux', 'num', 'det', 
                'prefix', 'suffix', 'm', 'nm'}
    with path.open("r", encoding="utf-8", newline="") as f:
        ...
        for row in reader:
            ...
            for line in lines:
                ...
                if (stripped.lower() in POS_TAGS and 
        ...
        try:
            ...
            for cfg in DICT_SOURCE_CONFIG:
                ...
                if layer:
                    ...
                    layer = seg.dictionary.add_layer(layer_name, priority=priority, source_name=cfg["name"])
                layer.entries.clear()
                try:
            ...
            seg.dictionary.rebuild_cache()
            print("[DEBUG] Dictionaries loaded into segmenter layers, initializing advanced segmenter (lmbrain)...")
...
def _build_grapheme_clusters(text: str) -> tuple[list[str], list[int]]:
    ...
    while i < n:
        ...
        if 0x1000 <= ord(ch) <= 0x109F and not (0x1040 <= ord(ch) <= 0x1049):
            ...
            while (
                i < n
                and not is_combining_mark(text[i])
    ...
    def _compute_cost_breakdown(seg_text: str, prev_word: str | None) -> dict:
        ...
        return {
            ...
            "components": {
                ...
                "lm_unigram_weighted": cfg.UNIGRAM_WEIGHT * float(lm_val if (lm_known or branch == "dict") else fallback_default),
                "unigram_weight": cfg.UNIGRAM_WEIGHT,
                "dict_discount": cfg.DICT_NO_LM_DISCOUNT if branch == "dict" and not lm_known else None,
                "oov_penalty": (syllables - 1) * cfg.OOV_SYLLABLE_PENALTY if branch in ("lm_only", "oov") else 0.0,
                "bigram_score": bigram_score_next if bigram_cost_next is not None else 0.0,
    ...
    while i < n:
        ...
        if best_piece is not None and best_piece_norm is not None:
            ...
            if g2p_data and g2p_data.get("syllables"):
                roman = " ".join(s.get("roman", "") for s in g2p_data["syllables"] if s.get("roman"))
            sub_entries.append(
                {
                    "head": shard,
                    "roman": roman,
...
def _apply_lm_weight_overrides(args) -> None:
    ...
    def _set_weight(attr: str, key: str) -> None:
        ...
        if SEGMENTER_INSTANCE is not None and getattr(SEGMENTER_INSTANCE, "config", None) is not None:
...
def _object_summary(name: str, obj, deep: bool = False, max_items: int = 20000,
                    count: Optional[int] = None, note: Optional[str] = None) -> dict:
...
def _collect_latent_memory(deep: bool = False, max_items: int = 20000) -> list[dict]:
    ...
    if seg is not None:
        d = seg.dictionary
        components.append(_object_summary("segmenter._all_words", getattr(d, "_all_words", None), deep, max_items))
        for lname, layer in d.layers.items():
            components.append(_object_summary(f"segmenter.layer.{lname}", layer.entries, deep, max_items,
    ...
    components.append(_object_summary("manual_text_overrides", MANUAL_TEXT_OVERRIDES, deep, max_items))
    ...
    if READING_SRS is not None:
        ...
        components.append(_object_summary("reading_srs.cards", READING_SRS.cards, deep, max_items,
                                          count=srs_count))
    else:
        components.append(_object_summary("reading_srs", None))

    components.append(_object_summary("stanza_ner", STANZA_NER, deep=False,
                                      note="pipeline object" if STANZA_NER is not None else None))
    components.append(_object_summary("ud_parser", UD_PARSER, deep=False,
                                      note="pipeline object" if UD_PARSER is not None else None))
    components.append(_object_summary("pos_tagger", POS_TAGGER, deep=False,
                                      note="pipeline object" if POS_TAGGER is not None else None))

    # LM brain tables (if loaded)
    try:
        import lmbrain  # type: ignore
        components.append(_object_summary("lmbrain.unigram_cost", getattr(lmbrain, "UNIGRAM_COST", None), deep, max_items))
        components.append(_object_summary("lmbrain.bigram_cost", getattr(lmbrain, "BIGRAM_COST", None), deep, max_items))
    except Exception:
    ...
    if adv is not None:
        components.append(_object_summary("advsegmenter.bk_tree", getattr(adv, "_bk_tree", None), deep, max_items))
        components.append(_object_summary("advsegmenter.spell_morph_cache", getattr(adv, "_spell_morph_cache", None), deep, max_items))
        components.append(_object_summary("advsegmenter.spell_vocab_size", getattr(adv, "_spell_vocab_size", None), deep=False))
    else:
        components.append(_object_summary("advsegmenter", None, note="advanced segmenter not initialized"))

    return components


def _mem_trace_start(path: str, query_len: Optional[int] = None, lite: Optional[bool] = None) -> Optional[dict]:
...
def _render_memory_html(proc: dict, components: list[dict], spikes: list[dict]) -> Response:
    ...
    spike_rows.append("<tr><th>Time</th><th>Path</th><th>Duration (ms)</th><th>RSS Δ</th><th>RSS Before</th><th>RSS After</th></tr>")
    for e in spikes:
...
def lookup():
    ...
    if use_raw:
        ...
        if exact_only:
            ...
            return jsonify({
                ...
                "ud_overlay": {"ok": False, "tokens": [], "edges": [], "roots": [], "doc2seg": [], "seg2doc": [-1], "error": "exact_only"},
            })
        segments = [q]
        island_spans = [(0, 1)]
    else:
    ...
    stanza_raw_ents, stanza_ner_entities = _run_stanza_ner_early(q, segments, temp_fills, island_spans)

    # Step 2: Full DP resegmentation with NER protection
    segments, fills_by_seg, island_spans, original_stanza_tokens = _dp_resegment_with_ner_protection(
    ...
    segments, fills_by_seg = _merge_consecutive_unknown_segments(segments, fills_by_seg, island_spans)
    ...
    if stanza_raw_ents:
        ...
        stanza_ner_entities = _filter_ner_entities_excluding_last_islands(segments, island_spans, stanza_ner_entities)
    ...
    grammar_overlay = {"tokens": [], "links": []} if lite_param else build_grammar_overlay_for_segments(segments, DICT)
    # 1e) Build UD dependency overlay (spaCy-based parser)
    ud_overlay = {"ok": False, "tokens": [], "edges": [], "roots": [], "doc2seg": [], "seg2doc": [], "error": "lite"} if lite_param else build_ud_overlay_for_segments(
        segments,
...
def api_fuzzy_smart():
    ...
    if not unknown_piece and not force_whole_token:
        return jsonify({"ok": False, "error": "missing_unknown_piece", "base_token": base_token}), 400
    ...
    if ADVANCED_SEGMENTER is None:
        return jsonify({"ok": False, "error": "advanced_segmenter_unavailable", "base_token": base_token}), 503
    ...
    def _distance_first(part: str, use_max_ed: int) -> list[dict]:
        ...
        if not part:
            return []
        try:
            return ADVANCED_SEGMENTER.suggest_spellings_distance_first(
                word=part,
            ...
            if not base:
                ...
                try:
                    ...
                    if g2p_result and g2p_result.get("syllables"):
                        roman = " ".join(s.get("roman", "") for s in g2p_result["syllables"] if s.get("roman"))
                except Exception:
                    pass
            enriched.append(
                {
    ...
    def _is_unknown_piece(p: dict) -> bool:
        ...
        return pos.startswith("unknown") or (len(senses) == 1 and isinstance(senses[0], str) and "no dictionary entry" in senses[0].lower())
    ...
    token_units = [base_token] if force_whole_token else (list(island_tokens) if island_tokens else [base_token])
    token_fills = None
    ...
    if force_whole_token:
        ...
        if 0 <= token_idx_in_island < len(token_units):
            if _token_contains_unknown_piece(token_fills[token_idx_in_island], unknown_piece) or token_units[token_idx_in_island] == unknown_piece:
    ...
    if target_token_idx is None:
        return jsonify({"ok": False, "error": "unknown_piece_not_found", "base_token": base_token}), 400
    ...
    run_heads = [pieces[i].get("head") for i in range(run_start, run_end + 1) if pieces[i].get("head")]
    ...
    def _try_span_search(
        ...
        left = "".join(pieces[i].get("head") for i in range(span_start, run_start) if pieces[i].get("head"))
        right = "".join(pieces[i].get("head") for i in range(run_end + 1, span_end + 1) if pieces[i].get("head"))
    ...
    def _build_span_tokens(span_start: int, span_end: int) -> list[str]:
        return [pieces[i].get("head") for i in range(span_start, span_end + 1) if pieces[i].get("head")]
    ...
    def _collect_groups(target_ed: int) -> list[dict]:
        ...
        groups.sort(key=lambda g: (int(g.get("span_len") or 0), len(g.get("span_text") or "")), reverse=True)
        # Global cap across all groups: prefer longer spans, then unigram cost within each.
        remaining = max_suggestions if isinstance(max_suggestions, int) and max_suggestions > 0 else 10
        trimmed: list[dict] = []
        for g in groups:
            if remaining <= 0:
                break
...
def _extract_pdf_pages_and_dims_from_bytes(data: bytes) -> tuple[list[str], list[dict], dict | None]:
            ...
            with pdfplumber.open(io.BytesIO(data)) as pdf:
                for p in pdf.pages:
                    try:
                        ...
                        pages.append("")
                    page_dims.append(_make_pdf_page_dim(getattr(p, "width", 0), getattr(p, "height", 0)))
            if pages:
...
def _extract_page_words_and_blocks(page, scale=1.0):
    ...
    for bno, block in enumerate(raw_blocks):
        ...
        for lno, line in enumerate(lines):
            ...
            def flush_run():
                nonlocal run_text, run_x0, run_y0, run_x1, run_y1, run_font_sizes, run_fonts, run_gap_before, run_space_before, word_idx, word_no
                if not run_text or run_x0 is None or run_y0 is None or run_x1 is None or run_y1 is None:
                    run_text = ""
                    run_x0 = run_y0 = run_x1 = run_y1 = None
                    run_font_sizes = []
                    run_fonts = []
        ...
        if abs(block_center - page_center) < center_tolerance and left_margin > margin_threshold and right_margin > margin_threshold:
            alignment = "center"
...
def debug_ud_parser():
    ...
    stanza_seg_param = str(data.get("stanza_segmenter") or "").strip().lower() in {"1", "true", "yes", "on"}
    if not text:
    ...
    if use_raw:
        ...
        if not norm:
            return jsonify({"ok": False, "error": "query contains no Burmese after normalization"}), 400
        ...
        if override_nlp is not None:
            ...
            dict_fills_for_kept = [fills_by_seg[si] for si in doc2seg] if doc2seg is not None else []
            for tok, fill_data in zip(doc, dict_fills_for_kept):
                if fill_data is not None:
                    tok._.dict_fills = fill_data
    ...
    try:
        for si, sent in enumerate(doc.sents):
            ...
            seg_start_i = doc2seg[start_i] if doc2seg is not None and start_i < len(doc2seg) else None
            seg_end_i = doc2seg[end_i - 1] if doc2seg is not None and end_i - 1 < len(doc2seg) else None
            sents_out.append(
                {
                    "i": int(si),
                    "start": start_i,
...
def debug_parser_route():
    ...
    stanza_seg_param = str(request.args.get("stanza_segmenter") or "").strip().lower() in {"1", "true", "yes", "on"}
        ...
        tokens_out.append(
            {
                ...
                "lemma": t.lemma_,
                "upos": t.pos_,
                "upos_original": morphologizer_output[t_idx] if morphologizer_output and t_idx < len(morphologizer_output) else None,
                "tag": t.tag_,
                "dep": t.dep_,
                "head": {"i": head_doc_i, "seg_i": head_seg_i, "text": t.head.text},
                "children": [int(ch.i) for ch in t.children],
    ...
    if segments is not None:
        segs_html = "<p><strong>Segments:</strong> " + " | ".join(html.escape(s) for s in segments) + "</p>"
        if segments_used is not None and segments_used != segments:
            segs_html += "<p><strong>Segments used for spaCy:</strong> " + " | ".join(html.escape(s) for s in segments_used) + "</p>"
    ...
    try:
        ...
        for si, sent in enumerate(sents):
            ...
            sent_lines.append(f"<li><strong>{si+1}.</strong> [{start_i},{end_i}){seg_part} {html.escape(sent.text)}</li>")
    ...
    try:
        ...
        if override_nlp_for_debug is not None and "dict_pos_override" in override_nlp_for_debug.pipe_names:
            ...
            for t in doc:
                ...
                if spacy_raw_scaled:
                    parts = [f"{k}:{spacy_raw_scaled[k]:.3f}" for k in sorted(spacy_raw_scaled.keys())]
                ...
                if spacy_scaled_scores:
                    parts = [f"{k}:{spacy_scaled_scores[k]:.3f}" for k in sorted(spacy_scaled_scores.keys())]
                ...
                show_breakdown = (
                    len(fills) > 1 or  # Multiple dictionary fills
                    subwords or        # Decomposition occurred
                    "decomposed" in source or  # Explicitly decomposed (e.g., Pali word)
                    "multi_fill" in source  # Compound with all valid POS (50/50 blend)
                )

                if show_breakdown and fills:
                    fparts = [f"{html.escape(f.get('head',''))}:{html.escape(f.get('pos',''))}" for f in fills]
                    fills_str = " | ".join(fparts)
                else:
                    fills_str = ""
                if show_breakdown and subwords:
                    sparts = [f"{html.escape(s.get('head',''))}:{html.escape(s.get('pos',''))}" for s in subwords]
                    subwords_str = " | ".join(sparts)
                else:
                    subwords_str = ""
                if show_breakdown and subwords2:
                    sparts2 = [f"{html.escape(s.get('head',''))}:{html.escape(s.get('pos',''))}" for s in subwords2]
                ...
                if show_breakdown and sub_debug:
                    ...
                    for sd in sub_debug:
                        ...
                        if s_pos_scores:
                            ...
                            score_parts = [f"{k}:{s_pos_scores[k]:.3f}" for k in sorted(s_pos_scores.keys())]
                    ...
                    subtable = (
                        "<table>"
                        "<tr><th>subword</th><th>allowed</th><th>" + col_header + "</th></tr>"
                        + "".join(sub_rows)
                        + "</table>"
                    )
                    calc_rows.append(
                        f"<tr><td colspan=\"12\">{subtable}</td></tr>"
                    )
            if calc_rows:
                override_html = (
...
def debug_displacy_route():
    ...
    try:
        ...
        return jsonify({"ok": False, "error": f"spacy_displacy_unavailable:{type(e).__name__}:{e}"}), 500
    ...
    if use_raw:
        ...
        if override_nlp is not None:
            ...
            doc = Doc(override_nlp.vocab, words=kept_words, spaces=[True] * (len(kept_words) - 1) + [False] if kept_words else [])
            if len(doc):
                doc[0].is_sent_start = True
            doc = override_nlp(doc)
        else:
            doc = parser._Doc(parser.nlp.vocab, words=kept_words, spaces=[True] * (len(kept_words) - 1) + [False] if kept_words else [])
            if len(doc):
                doc[0].is_sent_start = True
            doc = parser.nlp(doc)
    ...
    for result in results:
        ...
        if 'error' in result:
            ...
            if result.get('unigram'):
                ...
                """
                for pos, prob in sorted(uni['probabilities'].items(), key=lambda x: -x[1])[:5]:  # Top 5
                    count = uni['raw_counts'].get(pos, 0)
                ...
                if bi['probabilities']:
                    html_output += """
                        <table>
                            <tr><th>POS</th><th>Probability</th><th>Count</th><th>Distribution</th></tr>
                    """
                    for pos, prob in sorted(bi['probabilities'].items(), key=lambda x: -x[1])[:5]:  # Top 5
                        count = bi['raw_counts'].get(pos, 0)
                        bar_width = int(prob * 200)
                        pos_label = POS_DISPLAY_LABELS.get(pos, pos.upper())
                        html_output += f"""
        ...
        for p_idx, phrase in enumerate(phrases):
            ...
            for t_idx, seg in enumerate(phrase_segs):
                ...
                if seg in unknown_set:
                    ...
                    seg_html_tokens.append(
                        f'<span class="seg-known">{esc}</span>'
                    )
                # token separator inside each phrase
                if t_idx != n_segs - 1:
                    seg_html_tokens.append(
                        '<span class="seg-token-sep"> | </span>'
                    )
        html_parts: list[str] = []
        html_parts.append(
            "<!DOCTYPE html>"
